              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
                  ...
                  <MetricCard
                    icon={CheckCircle}
                    label="Program Fit"
                    value={achievedProgram?.fitScore != null ? `${achievedProgram.fitScore}%` : "—"}
                    sub={scenario.validationStatus === "pass" ? "validated" : "needs review"}
                  />
                </div>
                ...
                {/* Existing Conditions + Achieved-vs-Requested */}
                  ...
                  <div className="grid lg:grid-cols-2 gap-4">
                    <div className="bg-[#0F1F3D] border border-white/8 rounded-2xl p-5">
                      <div className="flex items-center justify-between gap-3 mb-3">
                        <span className="text-white font-['Montserrat'] font-700 text-sm">Existing Conditions</span>
                        <span className={`text-xs font-['Inter'] px-2 py-1 rounded-full border ${scenario.validationStatus === "pass" ? "text-emerald-300 border-emerald-400/25 bg-emerald-400/10" : "text-[#D4AF37] border-[#D4AF37]/25 bg-[#D4AF37]/10"}`}>
                          {existingConditions?.extractionStatus ?? scenario.validationStatus ?? "needs_review"}
                        </span>
                      </div>
                      <p className="text-white/60 font-['Inter'] text-sm leading-relaxed mb-3">
                        {existingConditions?.narrative ?? "Existing-condition inventory is not available for this scenario."}
                      </p>
                      {scenario.reuseStrategy && (
                        <div className="bg-white/3 border border-white/8 rounded-xl p-3 mb-3">
                          <div className="text-[#D4AF37] text-xs font-['Montserrat'] font-700 uppercase tracking-wider mb-1">Reuse Strategy</div>
                          <p className="text-white/65 text-xs font-['Inter'] leading-relaxed">{scenario.reuseStrategy}</p>
                        </div>
                      )}
                      {qaWarnings.length > 0 && (
                        <div className="space-y-1">
                          {qaWarnings.slice(0, 3).map((warning, i) => (
                            <p key={i} className="text-[#D4AF37]/85 text-xs font-['Inter'] leading-relaxed">Needs review: {warning}</p>
                          ))}
                        </div>
                      )}
                    </div>
                    ...
                    <div className="bg-[#0F1F3D] border border-white/8 rounded-2xl p-5">
                      <span className="text-white font-['Montserrat'] font-700 text-sm block mb-3">Achieved vs Requested Program</span>
                      <div className="space-y-2 max-h-72 overflow-auto pr-1">
                        {fitVariance.map((row, i) => (
                          <div key={i} className="grid grid-cols-[1fr_auto_auto_auto] gap-3 items-center py-2 border-b border-white/5 last:border-0">
                            <span className="text-white/75 text-xs font-['Inter'] truncate">{row.label}</span>
                            <span className="text-white/35 text-xs font-['Inter']">Req {row.requested}</span>
                            <span className="text-white/70 text-xs font-['Inter']">Got {row.achieved}</span>
                            <span className={`text-[10px] font-['Montserrat'] font-700 uppercase px-2 py-0.5 rounded-full ${row.status === "met" ? "bg-emerald-400/10 text-emerald-300" : row.status === "partial" ? "bg-[#D4AF37]/10 text-[#D4AF37]" : "bg-red-400/10 text-red-300"}`}>
                              {row.status}
                            </span>
                          </div>
                        ))}
                ...
                {changeSummary && (
                  <Accordion
                    title="Reuse, Repurpose, Demo Scope"
                    icon={CheckCircle}
                    id="scope"
                  ...
                  >
                    <div className="grid md:grid-cols-4 gap-3">
                      {[
                        { title: "Retained", items: changeSummary.retained ?? [] },
                        { title: "Repurposed", items: changeSummary.repurposed ?? [] },
                        { title: "New Build", items: changeSummary.newBuild ?? [] },
                        { title: "Demo", items: changeSummary.demolition ?? [] },
                      ].map((group) => (
                        <div key={group.title} className="bg-white/3 border border-white/8 rounded-xl p-3">
                          <div className="text-[#D4AF37] text-xs font-['Montserrat'] font-700 uppercase tracking-wider mb-2">{group.title}</div>
                          <ul className="space-y-1">
                            {group.items.slice(0, 4).map((item, i) => (
                              <li key={i} className="text-white/55 text-xs font-['Inter'] leading-relaxed">{item}</li>
                            ))}
                          </ul>
                        </div>
                      ))}
                ...
                {(scenario.layoutImageUrl || scenario.layoutSvg) && (
                  <div className="bg-[#0F1F3D] border border-white/8 rounded-2xl p-4">
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <span className="text-white/30 text-xs font-['Inter']">{scenario.layoutImageUrl ? 'AI Architectural Layout' : 'Conceptual diagram'}</span>
                    </div>
                    {scenario.layoutImageUrl ? (
                      <div className="w-full rounded-xl overflow-hidden bg-[#0A1628] flex items-center justify-center min-h-[280px]">
                        <img
