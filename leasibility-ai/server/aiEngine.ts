...
import {
  ...
  deriveRequestedProgram,
  extractExistingConditions,
  type AchievedProgram,
  type ExistingConditionsInventory,
  type FitVarianceRow,
  type RequestedProgram,
  type ScenarioPlanningMetadata,
} from "./planPipeline";
...
export interface GeneratedScenario {
  ...
  changeSummary: ScenarioPlanningMetadata["changeSummary"];
  qaWarnings: string[];
  validationStatus: ScenarioPlanningMetadata["validationStatus"];
  aiSummary: string;
}
...
export async function generateScenarios(input: ScenarioInput): Promise<GeneratedScenario[]> {
  ...
  const requestedProgram = deriveRequestedProgram({
    headcount: input.headcount,
    totalSqFt: input.totalSqFt,
    industry: input.industry,
    programNotes: input.programNotes,
  });
  const existingConditions = await extractExistingConditions({
...
Return JSON: { "scenarios": [ {...}, {...}, {...} ] }`;
  ...
  // ─── Build full scenario objects with AI image generation ─────────────────────────
  ...
  for (let idx = 0; idx < Math.min(aiResult.scenarios.length, 3); idx++) {
    ...
    // ─── Generate AI architectural floor plan image ───────────────────────────
    let layoutImageUrl: string | undefined = undefined;
    try {
      const roomList = s.roomBreakdown
        .map((r: { type: string; count: number; sqFt: number }) => `${r.count}x ${r.type} (${r.sqFt} sf each)`)
        .join(", ");

      // Progressive demolition rules — same building footprint, increasing intervention per scenario
      const demolitionRules = [
        // Scenario 1 — Light Refresh: keep ALL existing walls, work within them
        `CRITICAL ARCHITECTURAL RULE — LIGHT REFRESH SCENARIO:
You MUST preserve and redraw the EXACT same building perimeter, exterior walls, window locations, window mullions, entrance doors, stairs, elevators, restrooms, and all core building elements from the uploaded floor plan.
DO NOT change the building outline, shape, or footprint in any way whatsoever.
DO NOT demolish, move, or remove any existing interior partition walls.
Work ONLY within the existing room boundaries: assign new uses, add color-coded zone fills, and place furniture symbols inside existing rooms.
Show all existing walls as thick solid white lines. The building must look identical to the uploaded plan but with new colors and furniture.`,

        // Scenario 2 — Moderate Build-Out: keep perimeter and core, selectively demo interior walls
        `CRITICAL ARCHITECTURAL RULE — MODERATE BUILD-OUT SCENARIO:
You MUST preserve and redraw the EXACT same building perimeter, exterior walls, window locations, window mullions, entrance doors, stairs, elevators, restrooms, and all core building elements from the uploaded floor plan.
DO NOT change the building outline, shape, or footprint in any way whatsoever.
You MAY selectively demolish some existing interior partition walls to better fit the room program.
Show demolished walls as dashed red lines labeled DEMO. Show new partition walls as solid white lines labeled NEW.
The building perimeter must look identical to the uploaded plan. Only interior partitions may change.`,

        // Scenario 3 — Full Transformation: keep only perimeter and core, fully rebuild interior
        `CRITICAL ARCHITECTURAL RULE — FULL TRANSFORMATION SCENARIO:
You MUST preserve and redraw the EXACT same building perimeter, exterior walls, window locations, window mullions, entrance doors, stairs, elevators, restrooms, and all core building elements from the uploaded floor plan.
DO NOT change the building outline, shape, or footprint in any way whatsoever.
You MAY demolish ALL existing interior partition walls and rebuild the entire interior from scratch for the ideal layout.
Show all new partition walls as solid white lines. The building perimeter must look identical to the uploaded plan.
Create the optimal interior layout for the room program with no constraints from existing interior conditions.`,
      ];

      const scenarioRule = demolitionRules[idx] ?? demolitionRules[2];
      const hasFloorPlan = !!input.floorPlanUrl;

      const imagePrompt = hasFloorPlan
        ? `You are a licensed commercial interior architect creating a test fit space plan overlay for a tenant rep broker.

TASK: Using the uploaded floor plan image as your EXACT base, redraw it as a new test fit layout.

${scenarioRule}

ROOM PROGRAM TO FIT INTO THIS BUILDING: ${roomList}
TOTAL AREA: ${sqFt.toLocaleString()} sq ft
SCENARIO: ${impactTags[idx]} — ${s.label}

DRAWING STYLE:
- 2D top-down architectural floor plan, same orientation and scale as the uploaded plan
- Dark navy blue background #0F1F3D
- White exterior wall lines with 8-inch thickness — MUST match uploaded plan exactly
- White interior wall lines with 4-inch thickness
- Color-coded zone fills: Reception=gold #D4AF37, Open Workspace=royal blue #2563EB, Private Office=dark navy #1E3A5F, Conference Room=forest green #166534, Break Room=warm brown #78350F, Collaboration Zone=teal #0F766E, Phone Booth=slate #475569
- White quarter-circle door arc symbols at every room entrance
- White room name label and square footage number centered in each room
- White 5-foot-wide circulation corridor paths labeled CIRCULATION
- Furniture symbols: desks+chairs in offices, conference table+chairs in meeting rooms, kitchen counter+seating in break room
- Gold circular efficiency badge top-right: ${s.efficiencyScore}% EFFICIENCY SCORE
- Scale bar at bottom, label: LEASIBILITY AI — ${s.label.toUpperCase()} SCENARIO
- Professional broker presentation quality`
        : `You are a licensed commercial interior architect creating a test fit space plan for a tenant rep broker.

TASK: Create a 2D top-down architectural floor plan test fit. No existing floor plan was provided — generate a realistic rectangular office floor plate.

ROOM PROGRAM: ${roomList}
TOTAL AREA: ${sqFt.toLocaleString()} sq ft
SCENARIO: ${impactTags[idx]} — ${s.label}

DRAWING STYLE:
- 2D top-down architectural floor plan view
- Dark navy blue background #0F1F3D
- Realistic rectangular floor plate with white exterior walls 8-inch thickness
- Central H-shaped or spine circulation corridor 5 feet wide labeled CIRCULATION
- Color-coded zone fills: Reception=gold #D4AF37, Open Workspace=royal blue #2563EB, Private Office=dark navy #1E3A5F, Conference Room=forest green #166534, Break Room=warm brown #78350F, Collaboration Zone=teal #0F766E
- White interior wall lines 4-inch thickness with door arc symbols at every entrance
- White room name label and square footage number in each room
- Furniture symbols: desks+chairs in offices, conference table+chairs in meeting rooms, kitchen counter for break room
- Gold circular efficiency badge top-right: ${s.efficiencyScore}% EFFICIENCY SCORE
- Scale bar at bottom, north arrow, label: LEASIBILITY AI — ${s.label.toUpperCase()} SCENARIO
- Professional broker presentation quality`;

      const imageOptions: { prompt: string; originalImages?: Array<{ url: string }> } = { prompt: imagePrompt };
      if (input.floorPlanUrl) {
        imageOptions.originalImages = [{ url: input.floorPlanUrl }];
      }


      const imageResult = await generateImage(imageOptions);
      layoutImageUrl = imageResult.url;
      console.log(`[AI Engine] Floor plan image generated for scenario ${idx + 1}:`, layoutImageUrl);
    } catch (imgErr) {
      console.error(`[AI Engine] Image generation failed for scenario ${idx + 1}, using SVG fallback:`, imgErr);
    }
    ...
    finalScenarios.push({
      ...
      layoutSvg: generateSVGLayout(sqFt, s.roomBreakdown, impactLevel),
      layoutImageUrl,
      budgetLow:  totalLow,
      budgetMid:  totalMid,
      ...
      requestedProgram: metadata.requestedProgram,
      existingConditions: metadata.existingConditions,
      achievedProgram: metadata.achievedProgram,
      fitVariance: metadata.fitVariance,
      reuseStrategy: metadata.reuseStrategy,
      changeSummary: metadata.changeSummary,
      qaWarnings: metadata.qaWarnings,
      validationStatus: metadata.validationStatus,
      aiSummary: `${s.aiSummary}\n\nExisting-condition fit: ${metadata.reuseStrategy}`,
    });
  }
