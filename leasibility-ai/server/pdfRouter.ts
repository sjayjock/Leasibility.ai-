...
function buildReportHtml(
  ...
  const scenariosHtml = scenarioList.map((s) => {
    ...
    const fitVariance = (s.fitVariance as Array<{ label: string; requested: number; achieved: number; status: string }> | null) ?? [];
    const existingConditions = s.existingConditions as { extractionStatus?: string; narrative?: string } | null;
    const achievedProgram = s.achievedProgram as { fitScore?: number } | null;

    const fitRows = fitVariance.map(row => `
      <tr>
        <td style="padding:6px 8px;border-bottom:1px solid #e5e7eb;">${row.label}</td>
        <td style="padding:6px 8px;border-bottom:1px solid #e5e7eb;text-align:center;">${row.requested}</td>
        <td style="padding:6px 8px;border-bottom:1px solid #e5e7eb;text-align:center;">${row.achieved}</td>
        <td style="padding:6px 8px;border-bottom:1px solid #e5e7eb;text-align:center;color:${row.status === "met" ? "#15803d" : row.status === "partial" ? "#a16207" : "#b91c1c"};font-weight:700;text-transform:uppercase;">${row.status}</td>
      </tr>`).join("");
    ...
    <div style="page-break-before:always;padding:40px;">
      ...
      ${(existingConditions || fitRows || s.reuseStrategy) ? `
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;margin-bottom:32px;">
        <div style="border:1px solid #e5e7eb;border-radius:8px;padding:16px;background:#f8fafc;">
          <div style="font-size:12px;font-weight:700;color:#6b7280;text-transform:uppercase;letter-spacing:1px;margin-bottom:8px;">Existing Conditions</div>
          <p style="font-size:13px;color:#374151;line-height:1.5;margin:0 0 10px 0;">${existingConditions?.narrative ?? "Existing-condition inventory generated from uploaded plan context and intake assumptions."}</p>
          <div style="font-size:12px;color:#6b7280;margin-bottom:6px;">Validation: <strong style="color:${color};">${s.validationStatus ?? existingConditions?.extractionStatus ?? "needs_review"}</strong></div>
          ${s.reuseStrategy ? `<div style="font-size:12px;color:#374151;line-height:1.5;"><strong>Reuse strategy:</strong> ${s.reuseStrategy}</div>` : ""}
        </div>
        <div style="border:1px solid #e5e7eb;border-radius:8px;padding:16px;background:#f8fafc;">
          <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;">
            <div style="font-size:12px;font-weight:700;color:#6b7280;text-transform:uppercase;letter-spacing:1px;">Achieved vs Requested</div>
            <div style="font-size:18px;font-weight:800;color:${color};">${achievedProgram?.fitScore ?? "—"}% fit</div>
          </div>
          ${fitRows ? `<table style="width:100%;border-collapse:collapse;font-size:12px;"><thead><tr style="background:#0F1F3D;color:white;"><th style="padding:6px;text-align:left;">Program</th><th style="padding:6px;text-align:center;">Req.</th><th style="padding:6px;text-align:center;">Ach.</th><th style="padding:6px;text-align:center;">Status</th></tr></thead><tbody>${fitRows}</tbody></table>` : `<p style="font-size:13px;color:#6b7280;">Fit variance was not generated for this scenario.</p>`}
        </div>
      </div>` : ""}
      ...
      <div style="margin-bottom:32px;border:1px solid #e5e7eb;border-radius:8px;overflow:hidden;background:#f8fafc;padding:16px;">
        <div style="font-size:12px;font-weight:700;color:#6b7280;text-transform:uppercase;letter-spacing:1px;margin-bottom:12px;">Space Layout — AI Architectural Floor Plan</div>
        <img src="${s.layoutImageUrl}" alt="Floor plan for ${s.label}" style="width:100%;height:auto;border-radius:6px;display:block;" />
      </div>` : s.layoutSvg ? `
      <div style="margin-bottom:32px;border:1px solid #e5e7eb;border-radius:8px;overflow:hidden;background:#f8fafc;padding:16px;">
        <div style="font-size:12px;font-weight:700;color:#6b7280;text-transform:uppercase;letter-spacing:1px;margin-bottom:12px;">Space Layout</div>
        ${s.layoutSvg}
      </div>` : ""}
