...
- In Light Refresh (0-20% demo), accept existing rooms within 15% of canonical minimum size rather than demolishing for minor corrections. Minor demo is permitted when it meaningfully unlocks program fit — apply Wall Value Classification to determine which walls qualify.
- In Moderate Build-Out (30-60% demo), apply Wall Value Classification scoring to identify highest-value demo targets. Remove walls in descending score order until program target is met OR demo budget is reached — whichever comes first. Prefer minimum intervention.
- In Full Transformation (70-100% demo), remove all interior partitions except those scoring Very Low on the Wall Value Classification AND that directly satisfy a canonical room requirement without reconfiguration. Shell and core are always protected.
